...
class FirestoreDB():
    ...
    def load_data(self):
        if self.is_initialized:
            self._business_db = db.collection(
                "businesses").document(self._business_id)
            self._users_collection_ref = self._business_db.collection(
                self.collections["users"])
            self._emails_collection_ref = self._business_db.collection(
    ...
    def get_existing_message_ids(self) -> List[str]:
        ...
        if not doc.exists:
            if self._auto_init:
                ...
                raise RuntimeError(
                    "File 'existing_message_ids' does not exist!")
        return doc.to_dict()
