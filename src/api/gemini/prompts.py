...
def filter_spam_prompt(ref_products: List[Product], ref_company: str, messages: List[Message]) -> str:
    return f'''Analyze messages from random people if they are valuable feedback from users or not related to the listed products of the company '{ref_company}. You should return True if a message is a legit feedback and False if it is potentially a spam email or an unrelated message.'
